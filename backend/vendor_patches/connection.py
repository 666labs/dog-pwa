# ===========================================================================
# PATCHED COPY OF A PIP-INSTALLED dimOS FILE — NOT a normal module of this
...
#
#   /home/asus/dimos-env/lib/python3.12/site-packages/dimos/robot/unitree/connection.py
#
# (dev-laptop equivalent, which this copy was taken from:
#   /home/alex/miniconda3/envs/dimos/lib/python3.11/site-packages/dimos/robot/unitree/connection.py)
#
# It will be silently reverted by any `pip install`/upgrade of dimos —
# re-apply after one.
#
# CHANGES vs. upstream (both are DETECTION ONLY — no reconnect, no recovery,
# no movement, no change to what any existing caller observes):
#   1. raw_video_stream().accept_track() now catches aiortc's
#      MediaStreamError, logs the greppable sentinel GO2_WEBRTC_TRACK_DEAD,
#      and RE-RAISES. Re-raising preserves the exact prior behaviour for every
#      caller; the sentinel is a purely additive log line.
#   2. UnitreeWebRTCConnection gained a heartbeat dead-man watchdog thread
#      (start()/stop()) that logs the sentinel GO2_HEARTBEAT_DEAD when the
#      robot stops answering the vendored 2s heartbeat. See
#      _heartbeat_watchdog_loop for the full why.
# Both sentinels are consumed by backend/connection_watchdog.py, which tails
# this process's stdout and is the ONLY thing allowed to act on them.
# ===========================================================================
...
# --------------------------------------------------------------------------- #
# Heartbeat dead-man thresholds (see UnitreeWebRTCConnection._heartbeat_watchdog_loop).
#
# The vendored data channel sends a heartbeat every 2s and stamps
# `heartbeat_response` on every reply from the robot, so 6s = three replies
# missed in a row: long enough that one dropped datagram or a scheduler hiccup
# cannot trip it, short enough to beat the ~28s ICE consent-expiry floor that
# is otherwise the first thing in the whole stack to notice a dead session.
# --------------------------------------------------------------------------- #
HEARTBEAT_DEAD_THRESHOLD_S = 6.0
...
class UnitreeWebRTCConnection(Resource):
    ...
    def __init__(
        ...
        # Heartbeat dead-man watchdog: created here (before connect(), which can
        # raise) so stop()/disconnect() can always safely tear it down, started
        # in start(), stopped in stop()/disconnect().
        self._heartbeat_watchdog_stop = threading.Event()
        self._heartbeat_watchdog_thread: threading.Thread | None = None
    ...
    def start(self) -> None:
        # Only ever called after connect() has already succeeded, so the data
        # channel and its heartbeat exist by now.
        self._start_heartbeat_watchdog()

    # ----------------------------------------------------------------- #
    # Heartbeat dead-man watchdog — DETECTION ONLY
    # ----------------------------------------------------------------- #
    def _start_heartbeat_watchdog(self) -> None:
        if self._heartbeat_watchdog_thread is not None:
            return  # idempotent: a second start() must not spawn a second thread
        self._heartbeat_watchdog_stop.clear()
        self._heartbeat_watchdog_thread = threading.Thread(
            target=self._heartbeat_watchdog_loop,
            name="go2-heartbeat-watchdog",
            daemon=True,
        )
        self._heartbeat_watchdog_thread.start()

    def _stop_heartbeat_watchdog(self) -> None:
        """Signal and best-effort join. Never allowed to hang a shutdown: the
        thread is a daemon and the join is bounded, so a wedged poll costs at
        most DEFAULT_THREAD_JOIN_TIMEOUT and then gets abandoned."""
        stop_event = getattr(self, "_heartbeat_watchdog_stop", None)
        if stop_event is not None:
            stop_event.set()
        thread = getattr(self, "_heartbeat_watchdog_thread", None)
        self._heartbeat_watchdog_thread = None
        if thread is not None and thread.is_alive():
            thread.join(timeout=DEFAULT_THREAD_JOIN_TIMEOUT)

    def _last_heartbeat_response(self) -> float | None:
        """Wall-clock time the ROBOT last answered a heartbeat, or None.

        Defensive on purpose: the data channel object is created by connect()
        and replaced by the driver's reconnect(), so this attribute chain can
        legitimately be missing. A missing chain means "nothing to judge here",
        never "the link is dead" — this watchdog must not manufacture a death.
        """
        try:
            last = self.conn.datachannel.heartbeat.heartbeat_response
        except AttributeError:
            return None
        return last if isinstance(last, (int, float)) else None

    def _heartbeat_watchdog_loop(self) -> None:
        """Poll the vendored heartbeat's last-reply timestamp and log ONE loud,
        greppable line when the robot stops answering.

        WHY THIS EXISTS: the driver already sends a heartbeat every 2s and
        stamps `heartbeat_response` on every reply — and until this loop,
        NOTHING IN THE ENTIRE STACK EVER READ IT. When the WebRTC session dies
        silently (RF degradation -> aioice RFC 7675 consent-freshness expiry ->
        the one shared DTLS transport under video/lidar/cmd_vel torn down), the
        first thing that notices anything is the ~28s consent-expiry floor, and
        the sensor Observables simply stop emitting without erroring. Reading
        the heartbeat that is already there turns that into ~6s detection for
        free, with no new traffic on the wire.

        SCOPE — DETECTION ONLY. This thread logs and does nothing else: no
        disconnect(), no reconnect(), no touching connection state, no movement
        of any kind. Recovery is deliberately somebody else's job (the external
        connection_watchdog, which tails this log for GO2_HEARTBEAT_DEAD), so
        that a false positive here can never do anything worse than write a
        line to a log.

        The report is EDGE-triggered: once per staleness episode, re-armed only
        when a fresh response actually arrives. Level-triggering would emit a
        line every second for as long as the link stayed dead, which is exactly
        when the log most needs to stay readable.
        """
        reported_dead = False
        while not self._heartbeat_watchdog_stop.is_set():
            # wait() rather than sleep() so stop() is responsive within one poll.
            if self._heartbeat_watchdog_stop.wait(HEARTBEAT_POLL_INTERVAL_S):
                return

            last = self._last_heartbeat_response()
            if last is None:
                # No reply has EVER arrived on this session. That is connection
                # warmup, not a death (and a session that never comes up at all
                # fails loudly in connect()) — so there is nothing to declare
                # dead yet. This is also the whole startup-grace mechanism: the
                # check simply does not arm until the robot has spoken once.
                continue

            age = time.time() - last
            if age > HEARTBEAT_DEAD_THRESHOLD_S:
                if not reported_dead:
                    reported_dead = True
                    logger.error(
                        "GO2_HEARTBEAT_DEAD: no heartbeat response in %.1fs "
                        "(last response at %s)",
                        age,
                        time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(last)),
                    )
            elif reported_dead:
                # A fresh reply after a reported episode: re-arm, so a SECOND
                # death later in the same session reports again.
                reported_dead = False
                logger.info("heartbeat responses resumed (age %.1fs)", age)

    def stop(self) -> None:
        # Kill the dead-man watchdog FIRST: everything below is a deliberate
        # teardown, and heartbeats stopping because we asked them to must not
        # be reported as a connection death (a spurious GO2_HEARTBEAT_DEAD
        # would make the external watchdog "recover" a link we just closed).
    ...
    def raw_video_stream(self) -> Observable[VideoMessage]:
        ...
        async def accept_track(track: MediaStreamTrack) -> None:
            ...
            try:
                ...
                # The ONE exception that means the session is gone for good: a
                # stopped track cannot be restarted, and the DTLS transport it
                # sat on is shared with lidar/odom/cmd_vel, so they are gone
                # too. Everything downstream of `subject` just stops emitting
                # (no on_error is wired anywhere), which is precisely why this
                # death was invisible. Re-raised unchanged — behaviour is
                # identical to before; the sentinel is purely an added log line
                # for the external connection_watchdog to grep for.
                logger.error(
                    "GO2_WEBRTC_TRACK_DEAD: video track ended irreversibly "
                    "(ICE consent expiry or remote DTLS teardown) — session "
                    "requires a full reconnect, nothing will recover it in place"
                )
                raise
    ...
    def disconnect(self) -> None:
        ...
        # Same reason as in stop(): a deliberate teardown must not be reported
        # as a heartbeat death. disconnect() is reachable without stop().
        self._stop_heartbeat_watchdog()
